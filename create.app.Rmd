...
set.seed(99)
test <- data.frame(matrix(nrow = 348, ncol = 5))
colnames(test) <- c("age", "uni", "hs", "countryyr")
test$age <- rep(seq(from = 18, to = 75, by = 1), 6)
test$uni <- rep(c(1,0,0,1,0,0), each = 58)
test$hs <- rep(c(0, 1, 0,0,1,0), each = 58)
test$category <- rep(c("College", "HS", "Low Edu", "College", "HS", "Low Edu"), each = 58)
test$countryyr <- rep(c("u", "z"), each = 174)
test$predictedprob <- ifelse(test$uni == 1, (test$age - 18)*0.01, ifelse(test$hs == 1, (test$age - 18)*0.015, (test$age - 15)*0.015))
test$predictedprob <- ifelse(test$countryyr == "z", test$predictedprob- 0.15, test$predictedprob)
test$LL <- test$predictedprob - 0.2
test$UL <- test$predictedprob + 0.2
                          

shinyApp(
        ui = fluidPage(
                inputPanel(selectInput("cyr", label = "Choose a country:",choices = c("u" = "u", "z", "z"),
        ...
        server = function(input, output) {
                ...
                datasetInput <- reactive({
                        selection <- switch(input$yr,
                                   "u" = "u", "z" = "z")
                        subset(test, countryyr == selection)         
